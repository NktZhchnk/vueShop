...
<template>
  <div class="order-details">
    <h2>Order Details</h2>
    <div style="width: 100%; margin-top: 25px; display: flex; justify-content: space-between; align-items: center">
      <div>
    ...
    <div v-for="item in order" :key="item.id" class="order-item">
      ...
      <template v-if="item.variety_name !== null">
        <div style="display: flex">
          ...
          <div style="margin-left: 20px">
            <p class="variety-info">Назва: {{ item.variety_name }}</p>
            <p class="variety-info">Кількість: {{ item.quantity }}</p>
            <p class="variety-info">Ціна: {{ item.variety_price }} ₴</p>
          </div>
        </div>
      </template>
      ...
      <template v-else>
        <div style="display: flex">
          ...
          <div style="margin-left: 20px">
            <p class="item-info">Назва: {{ item.item_name }}</p>
            <p class="item-info">Кількість: {{ item.quantity }}</p>
            <p class="item-info">Ціна: {{ item.price }} ₴</p>
          </div>
        </div>
      </template>
    </div>
...
const removeOrder = async () => {
  ...
  for (const item of order.value) {
    ...
    if (item.order_product_id) {
      ...
      shouldDelete = true; // Установка флага для удаления
    }
